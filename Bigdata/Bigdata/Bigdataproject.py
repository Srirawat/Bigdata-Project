...
def show_profile(name, student_id, major, interest, experience, skills, img_url=None):
    ...
    if not img_url:
        ...
        with c2:
            url = st.text_input("หรือใส่ URL ของรูป (http/https)")
            if url:
                img_url = url
...
# ================== ROUTING ==================
if page == "profile":
    # This path is relative to where you run `streamlit run mix.py`
    candidate = first_existing_path(["FUJI0041.jpg", "img/FUJI0041.jpg"])
    my_img = image_to_base64(candidate) if candidate else None
    show_profile(
